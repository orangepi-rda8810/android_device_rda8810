...
#ifndef TARGET_ALL_PARTITIONS_ON_FTL
	if (memcmp(mtd->name, "fat", strlen("fat"))) {
		printk(KERN_INFO"rftl : %s : part name is %s, not support now, ignore.\n", __func__, mtd->name);
		return;
	}
	index = 'z'-'a';
#else
	if (memcmp(mtd->name, "fat", strlen("fat")) == 0)
			index = 'z'-'a';
	else if (memcmp(mtd->name, "system", strlen("system")) == 0)
			index = 'y'-'a';
	else if (memcmp(mtd->name, "userdata", strlen("userdata")) == 0)
			index = 'x'-'a';
	else if (memcmp(mtd->name, "vendor", strlen("vendor")) == 0)
			index = 'w'-'a';
	else if (memcmp(mtd->name, "cache", strlen("cache")) == 0)
			index = 'v'-'a';
