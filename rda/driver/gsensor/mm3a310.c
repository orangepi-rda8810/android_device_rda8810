...
/* driver version info*/
#define DRIVER_VERSION "2013-09-05-17:41:22_1.5.0"
...
static int Log_level = DEBUG_ERR; //|DEBUG_MSG|DEBUG_FUNC|DEBUG_DATA; 

#define MI_DATA(format, ...)  if(DEBUG_DATA&Log_level){printk(KERN_INFO MI_TAG "[DATA_S] " format " [DATA_E]" "\n", ## __VA_ARGS__);}
#define MI_MSG(format, ...)   if(DEBUG_MSG&Log_level){printk(KERN_INFO MI_TAG format "\n", ## __VA_ARGS__);}
#define MI_ERR(format, ...)   if(DEBUG_ERR&Log_level){printk(KERN_INFO MI_TAG format " in function %s  is called, line: %d\n", ## __VA_ARGS__, __FUNCTION__,__LINE__);}
#define MI_FUN                      if(DEBUG_FUNC&Log_level){printk(KERN_INFO MI_TAG "%s  is called, line: %d\n", __FUNCTION__,__LINE__);}
...
#define TARGET_PLATFORM					PLATFORM_ELSE//PLATFORM_QUACOMM//PLATFORM_STAND
...
#else
    #define MM3A310_INPUT_DEV_NAME "mm3a310"  //this name should be compatible with the define in HAL
#endif
...
#define MM3A310_I2C_ADDR    0x27 /* When SA0=1 then 27. When SA0=0 then 26*/
...
#elif TARGET_PLATFORM == PLATFORM_STAND
...
static int filter_average_enhance(FilterAverageContext* fac, short sample)
{
    ...
    if((abs(fac->sample_h)<60)&&(abs(fac->sample_h)>20))
	{
        fac->sample_h=(fac->sample_h>0)?(fac->sample_h-20):(fac->sample_h+20);
	}
...
static int mm3a310_set_enable(bool bEnable){
    ...
    if (val < 0){
        ...
        if(bEnable)
        {  
            ret = mm3a310_write_register(client, MM3A310_TEMP_CFG_REG, 0x88); 
        }else{        
...
#else
   input_report_abs(mm3a310_idev->input, ABS_X, -y);
    input_report_abs(mm3a310_idev->input, ABS_Y, x);
...
static DEVICE_ATTR(enable,      S_IRUGO | S_IWUSR | S_IWGRP,  mm3a310_enable_show, mm3a310_enable_store);
static DEVICE_ATTR(delay,      S_IRUGO | S_IWUSR | S_IWGRP,  mm3a310_delay_show, mm3a310_delay_store);
////static DEVICE_ATTR(poll,      S_IRUGO | S_IWUGO,  mm3a310_delay_show, mm3a310_delay_store);
static DEVICE_ATTR(offset,      S_IWUSR | S_IWGRP | S_IRUGO,  mm3a310_offset_show, mm3a310_offset_store);
static DEVICE_ATTR(aix_data,    S_IRUGO,            mm3a310_aix_data_show, NULL);
static DEVICE_ATTR(reg_data,    S_IWUSR | S_IWGRP | S_IRUGO,  mm3a310_reg_data_show, mm3a310_reg_data_store);
static DEVICE_ATTR(grange,      S_IWUSR | S_IWGRP | S_IRUGO,  mm3a310_grange_show, mm3a310_grange_store);
static DEVICE_ATTR(odr,         S_IWUSR | S_IWGRP | S_IRUGO,  mm3a310_odr_show, mm3a310_odr_store);
#if MM3A310_OFFSET_TEMP_SOLUTION
static DEVICE_ATTR(calibrate_miraGSensor,   S_IWUSR | S_IWGRP | S_IRUGO,            mm3a310_calibrate_show, mm3a310_calibrate_store);
static DEVICE_ATTR(linearity,  0660,  mm3a310_linearity_show, mm3a310_linearity_store);
#endif
#if FILTER_AVERAGE_ENHANCE
static DEVICE_ATTR(average_enhance,   S_IWUSR | S_IWGRP |S_IRUGO, mm3a310_average_enhance_show, mm3a310_average_enhance_store);
#endif /* ! FILTER_AVERAGE_ENHANCE */
...
//    &dev_attr_poll.attr, 
	...
	static struct device_attribute* mm3a310_device_attributes[] = { 
	 ...
	 &dev_attr_poll_delay,
	    ...
	    &dev_attr_odr,
	    #if MM3A310_OFFSET_TEMP_SOLUTION   
	    &dev_attr_calibrate_miraGSensor,
	            &dev_attr_linearity,
	            #endif
                   ...
                   const struct i2c_device_id *id)
{
    ...
    mdelay(50);
    mm3a310_write_register(client, MM3A310_PAGE_NO, 0x01);
    MI_MSG("check MM3A310 chip ID2\n");
    result = mm3a310_read_register(client, MM3A310_WHO_AM_I);
    MI_ERR("read chip ID result=0x%x\n", result);
    if (MM3A310_ID != (result&0xFF)) {    //compare the address value 
        MI_ERR("read chip ID 0x%x is not equal to 0x%x!", result,MM3A310_ID);
        result = -EINVAL;
        ////goto err_detach_client; ////James20130929.
    }
    ...
    idev->name = "rda-gsensor";////MM3A310_INPUT_DEV_NAME;////James20131014
	idev->uniq = "mm3a310";
...
static void mm3a310_early_resume(struct early_suspend* es)
{
    ...
    result = mm3a310_write_register(mm3a310_i2c_client, MM3A310_TEMP_CFG_REG,  0x88);//mm3a_status.temp_cfg_reg);
    MI_ASSERT(result==0);
    return;
}
#endif
...
#ifdef CONFIG_PM
static int mm3a310_suspend(struct device *dev)
{
    int result;
    mm3a_status.temp_cfg_reg = mm3a310_read_register(mm3a310_i2c_client, MM3A310_TEMP_CFG_REG);
    result = mm3a310_write_register(mm3a310_i2c_client, MM3A310_TEMP_CFG_REG, mm3a_status.temp_cfg_reg | 0x20); /* Power down enable */
    MI_ASSERT(result==0);
    result = 0x00;
    result = mm3a310_read_register(mm3a310_i2c_client, MM3A310_TEMP_CFG_REG);
    printk("suspend mm3a310_read_register here...MM3A310_TEMP_CFG_REG=0x%x\n", result);
    return 0;
}

static int mm3a310_resume(struct device *dev)
{
    int result;
    result = mm3a310_write_register(mm3a310_i2c_client, MM3A310_TEMP_CFG_REG, 0x88);///mm3a_status.temp_cfg_reg
    MI_ASSERT(result==0);
    result = 0x00;
    result = mm3a310_read_register(mm3a310_i2c_client, MM3A310_TEMP_CFG_REG);
    printk("resume mm3a310_read_register here...MM3A310_TEMP_CFG_REG=0x%x\n", result);
    return 0;
}
#endif
...
static SIMPLE_DEV_PM_OPS(mm3a310_pm_ops, mm3a310_suspend, mm3a310_resume);
...
#else
/*********************************************************************************/
	i2c_dev_gsensor.addr =  _DEF_I2C_ADDR_GSENSOR_MM3A310;

	adapter = i2c_get_adapter(_TGT_AP_I2C_BUS_ID_GSENSOR);

	if (!adapter) {
		MI_ERR("%s, cannot get i2c adapter %d\n",
	...
	if (res < 0) {
		MI_ERR(KERN_INFO "add mma7660 i2c driver failed\n");
		return -ENODEV;
	}

	MI_MSG("rda_gs %s initialized, at i2c bus %d addr 0x%02x\n",
		"mm3a310", _TGT_AP_I2C_BUS_ID_GSENSOR, 
