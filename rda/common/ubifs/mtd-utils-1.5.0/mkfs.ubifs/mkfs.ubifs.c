...
#define MAX_PATH 255
#define MAX_XATTR_ITEMS    64
#define SELINUX_UNDEFINED_LABLE "system_u:object_r:undefined_t:s0"
/**
...
unsigned int source_path_len = 0;
...
/**
 * fix file or dir stat for android system
 */
static void fix_stat(const char *path, struct stat *s)
{
	char * entry = NULL;
	uint64_t capabilities;
	entry = path + source_path_len;
#if ANDROID_SDK_VERSION >= 23
	fs_config(entry, S_ISDIR(s->st_mode),root,&s->st_uid, &s->st_gid, &s->st_mode,&capabilities);
#else
	fs_config(entry, S_ISDIR(s->st_mode), &s->st_uid, &s->st_gid, &s->st_mode,&capabilities);
#endif
	//fprintf(stderr, "fix_stat %s --- gid = %d, uid = %d \n", path, s->st_gid, s->st_uid);
}

static void get_capabilities(const char *path, uint64_t *capabilities)
{
	struct stat states;
	struct stat *s;
	char * entry = NULL;
	...
	s = &states;
#if ANDROID_SDK_VERSION >= 23
	fs_config(entry, 0, root, &s->st_uid, &s->st_gid, &s->st_mode, capabilities);
#else
	fs_config(entry, 0, &s->st_uid, &s->st_gid, &s->st_mode, capabilities);
#endif
}

/*
 * create a buffer of data based on input capabilities
 * return the data in buf, and return the length of the buf
 * return negtive value in case of errors
 * not sure whethere this is the right way to add xattr,
 * reverse engineering from linux kernel
 * TODO - add getcap for busybox, so we can get xattr easily from shell
 */
static int create_capabilities(uint64_t capabilities, char *buf)
{
	struct __user_cap_data_struct *data;

	data = (struct __user_cap_data_struct *)buf;
	memset(buf, 0, sizeof(struct __user_cap_data_struct));

	data->effective = VFS_CAP_REVISION_1;
	data->permitted = capabilities;
	data->inheritable = capabilities;

	return sizeof(struct __user_cap_data_struct);
...
static int calc_min_log_lebs(unsigned long long max_bud_bytes)
{
	...
	if (is_power_of_2(c->min_io_size))
		...
		log_size = UBI_ALIGN(UBIFS_REF_NODE_SZ, c->min_io_size);
	log_size *= buds;
	if (is_power_of_2(c->min_io_size))
		...
		log_size += UBI_ALIGN(UBIFS_CS_NODE_SZ +
				UBIFS_REF_NODE_SZ * (c->jhead_cnt + 2),
				c->min_io_size);
...
static int get_options(int argc, char**argv)
{
	...
	while (1) {
		...
		case 'S':
			...
			if (!sehnd) {
				return err_msg("failed to initialize selinux context");
			}
			printf("selinux context setup to file %s\n", optarg);
			break;
		}
	}
...
{
	int pad_len, alen = ALIGN(len, 8), wlen;
	uint32_t crc;

	if (is_power_of_2(c->min_io_size))
		wlen = ALIGN(alen, c->min_io_size);
	else
		wlen = UBI_ALIGN(alen, c->min_io_size);
...
static void set_lprops(int lnum, int offs, int flags)
{
	...
	if (is_power_of_2(a))
		...
		free = c->leb_size - UBI_ALIGN(offs, a);
 ...
 * @flags: source inode flags
 */
static int add_inode_with_xattr(struct stat *st, ino_t inum, struct xattr_entry *xattr_data,
			   unsigned int items, int flags)
{
	struct ubifs_ino_node *ino = node_buf;
	...
	ino_t org_inum;
	struct qstr nm;
	int xattr_size, xattr_names;
	int i;

	dbg_msg(1, "%s for inode %d\n", __func__, inum);
#if 0
	for (i=0; i<items; i++) {
		fprintf(stderr, " %s - %s \n",
		xattr_data[i].name , xattr_data[i].value);
	}
#endif
	xattr_size = xattr_names = 0;
	org_inum = inum;
	...
	ino->xattr_cnt = items;
	for (i=0; i<items; i++) {
		name = xattr_data[i].name;
		xattr_size += CALC_DENT_SIZE(strlen(name));
		xattr_size += CALC_XATTR_BYTES(xattr_data[i].value_len);
		xattr_names += strlen(name) + 1;
	}
	ino->xattr_size = xattr_size;
	ino->xattr_names = xattr_names;
	...
	for (i=0; i<items; i++) {
		...
		xent_key_init(c, &xkey, org_inum, &nm);
		...
		ino->creat_sqnum = cpu_to_le64(creat_sqnum);
		ino->size = cpu_to_le64(xattr_data[i].value_len);
		/*
		 * The time fields are updated assuming the default time granularity
		 * of 1 second. To support finer granularities, utime() would be needed.
		 */
		...
		ino->data_len = cpu_to_le32(xattr_data[i].value_len);
		ino->compr_type = cpu_to_le16(c->default_compr);


		ino->xattr_cnt = 1;
		ino->xattr_size = xattr_data[i].value_len;
		ino->xattr_names = strlen(xattr_data[i].name) + 1;
		if (xattr_data[i].value_len)
			memcpy(&ino->data, xattr_data[i].value, xattr_data[i].value_len);

		len = UBIFS_INO_NODE_SZ + xattr_data[i].value_len;
		ret = add_node(&nkey, NULL, ino, len) ;
		if (ret) {
			printf("new inode add failed\n");
			return ret;
		}
		free(xattr_data[i].value);
	}
	return ret;
}
...
static int add_dir_inode(DIR *dir, ino_t inum, loff_t size, unsigned int nlink,
			 struct stat *st, char *path_name)
{
	int fd, flags = 0;
	struct xattr_entry xattrs[MAX_XATTR_ITEMS];
	int xattr_items = 0;
	char *secontext;
	...
	if (sehnd) {
		...
		snprintf(full_path, MAX_PATH, "/%s", path_name+source_path_len);
		if (selabel_lookup(sehnd, &secontext, full_path, st->st_mode) < 0) {
			secontext = strdup(SELINUX_UNDEFINED_LABLE);
			printf("%s is using undefined lable\n", full_path);
		}
		xattrs[xattr_items].name = XATTR_NAME_SELINUX;
		xattrs[xattr_items].value = secontext;
		xattrs[xattr_items].value_len = strlen(secontext) + 1;
		xattr_items++;
		//fprintf(stderr, "%s - selable %s\n", full_path, secontext);
	}

	if (xattr_items)
		return add_inode_with_xattr(st, inum, xattrs, xattr_items, flags);
	return add_inode(st, inum, flags);
}
			...
			int flags)
{
	...
	{
		uint64_t capabilities;
		get_capabilities(path_name, &capabilities);
		if (capabilities != 0) {
			dbg_msg(1, "caps %d for file %s\n", capabilities, path_name);
			caps = malloc(sizeof(struct __user_cap_data_struct));
			if (caps == NULL) {
				close(fd);
				return sys_err_msg("failed to alloc buffer %s - %d",
					__func__, __LINE__);
			}
			xattr_len = create_capabilities(capabilities, caps);
			if (xattr_len < 0) {
				close(fd);
				free(caps);
				return sys_err_msg("failed to create_capabilities %s, %d\n",
						__func__, xattr_len);
			}
			xattrs[xattr_items].name = XATTR_NAME_CAPS;
			xattrs[xattr_items].value = caps;
			xattrs[xattr_items].value_len = xattr_len;
			xattr_items++;
			fprintf(stderr,"add capability for file %s\n",
				       path_name + source_path_len);
...
static int write_data(void)
{
	...
	if((len >= 4) && (!strcmp(root + len - 4, "data") || !strcmp(root + len - 5, "data/"))) {
		if(root[len-1] == '/')
			source_path_len = len - 5;
		else
			source_path_len = len - 4;
	} else if((len >= 6) && (!strcmp(root + len - 6, "system") || !strcmp(root + len - 7, "system/"))) {
		if(root[len-1] == '/')
			source_path_len = len - 7;
		else
			source_path_len = len - 6;
	} else if((len >= 6) && (!strcmp(root + len - 6, "vendor") || !strcmp(root + len - 7, "vendor/"))) {
		if(root[len-1] == '/')
			source_path_len = len - 7;
		else
			source_path_len = len - 6;
	} else if((len >= 6) && (!strcmp(root + len - 5, "cache") || !strcmp(root + len - 6, "cache/"))) {
		if(root[len-1] == '/')
			source_path_len = len - 6;
		else
			source_path_len = len - 5;
	} else { 
		fprintf(stderr, "mkubifs : fix filesystem stat : unknown file system name, exit!\n");
		return -1;
	}
...
static int write_index(void)
{
	...
	if (is_power_of_2(c->min_io_size))
		...
		c->ihead_offs = UBI_ALIGN(head_offs, c->min_io_size);
